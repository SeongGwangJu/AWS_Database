...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `get_product_by_category_case2` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_product_by_category_case2`(
	in vs_category_name varchar(50) 
)
BEGIN
        case vs_category_name
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_product_total_count`(
    ...
    out vn_total_count int, #return. 이걸 어떻게 call하는지 잘 볼것.
    inout vs_color_name varchar(50)
)
BEGIN
	#따라 친 코드니까, 원리를 정확히 파악할것!! inout개념도 복습필요 . 
    set vs_color_name = 'red';
	select
		count(*) into vn_total_count #상품이 몇개 있는지 계산해서 vn_count라는 변수에 담음
	from
		product_tb pt
